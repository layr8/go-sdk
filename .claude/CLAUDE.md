...
- The Go SDK auto-adds the problem report protocol (`https://didcomm.org/report-problem/2.0`) in `handlerRegistry.payloadTypes()`, so even handler-less clients always have at least one protocol.
...
- If a `Protocols` config field is added in the future, it should merge with (not replace) handler-derived protocols, matching the Python SDK pattern.

## Conventions

- Use `context.Context` propagation throughout
- Functional options pattern for API methods (`WithNonce()`, `WithVerifierDID()`, etc.)
- Custom `DialContext` in `http.Transport` for `*.localhost` resolution (RFC 6761)
